    ...
    var server = httpolyglot.createServer({
        ...
        if (req.url.indexOf('/login') === 0) {
            ...
            if (url.indexOf('redirectTo=') !== -1) {
                ...
                url = 'https://' + server + '/login?redirectTo=' + url;
            }

            res.statusCode = 302;
            res.setHeader('location', url);
            console.log('detected login redirect url, bouncing to: ' + url);
            return res.end();
        }

        return proxy.web.apply(proxy, modeFn(req, res));
    });
